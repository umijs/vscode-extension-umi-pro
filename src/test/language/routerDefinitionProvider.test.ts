...
describe('test UmiRouterDefinitionProvider', async () => {
  ...
  it('should get correct page path', async () => {
    ...
    const definition = await provider.provideDefinition(text, new Position(35, 35));
    assert.equal(!!definition, true);
    assert.equal(definition!.uri.fsPath, getAntdProFilePath('src/pages/Dashboard/Analysis.js'));
  });

  it('should get correct layout path', async () => {
    const text = await workspace.openTextDocument(getAntdProFilePath('config/router.config.js'));
    const definition = await provider.provideDefinition(text, new Position(4, 27));
    assert.equal(!!definition, true);
    assert.equal(definition!.uri.fsPath, getAntdProFilePath('src/layouts/UserLayout.js'));
  });
});
