...
describe('test TextDocumentUtils', () => {
  ...
  it('test CharAt', async () => {
    const text = await vscode.workspace.openTextDocument(join(fixtures, 'offsetChar'));
  ...
  it('test growBracketsRange', async () => {
    const text = await vscode.workspace.openTextDocument(join(fixtures, 'growBracketsRange.js'));
    const textDocumentUtils = new TextDocumentUtils(text);
    const range = textDocumentUtils.growBracketsRange(new vscode.Position(20, 17), Brackets.CURLY);
  ...
  it('test growBracketsRange', async () => {
    const text = await vscode.workspace.openTextDocument(join(fixtures, 'getQuoteRange.js'));
    const textDocumentUtils = new TextDocumentUtils(text);
    let range = textDocumentUtils.getQuoteRange(new vscode.Position(3, 18), QuoteType.single);
    assert.equal(new vscode.Range(3, 15, 3, 19).isEqual(range!), true);
    range = textDocumentUtils.getQuoteRange(new vscode.Position(4, 70), QuoteType.single);
